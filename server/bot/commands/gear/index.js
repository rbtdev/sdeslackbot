...
var desc =
	...
	"after 72 hours if no matching post has been submitted.  If someone submits a " +
...
function listResponse(userId, respond) {
	return function (err, results) {
		...
		if (err) return respond({
			text: "Unable to get a list of gear posts."
		})
		if (results && results.length == 0) return respond({
			text: "No active gear posts."
		});
		var attachments = [];
		var text = ""
...
function postResponse(respond) {
	return function (err, post) {
		...
		if (post.action == "need") {
			respond({
				text: "Your gear needs have been submitted. You'll receive a message in your Slackbot channel when someone has what you need. Gear exchange requests expire in 72 hours."
			})
		} else if (post.action == "have") {
			return respond({
				text: "Your gear availability has been submitted. You'll receive a message in your Slackbot channel when someone needs what you have. Gear exchange requests expire in 72 hours."
			})
		} else return respond({
			text: "Thank you for using the Intel Gear Exchange"
		})
	}
};
...
function deletePost(user, qualifier, item, respond) {
	GearController.delete(user.id, qualifier, item, function (err) {
		if (err) return respond({
			text: "Cannot find a post for " + qualifier + " " + item
		})
		return respond({
			text: "Deleted your post for " + qualifier + " " + item
		})
	})
}
...
function gear(command) {
	...
	var request = args._;
	if (request.length < 1) return respond({
		text: "Usage: " + gearHelp
	});
	...
	if (!fullAction.err) {
		switch (fullAction.action) {
			...
			default:
				return respond({
					text: "need valid action: " + actions
				});
				break
		}
	} else {
...
function getFullAction(request) {
	...
	if (action == "need" || action == "have" || action == "del") {
		...
		return ({
			err: "No such thing as a " + qualifier + " " + item
		});
	} else {
		return ({
			err: null,
