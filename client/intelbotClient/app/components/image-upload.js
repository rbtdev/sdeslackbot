...
export default Ember.Component.extend({
  ...
  attachments: [],
  errors: [],
  uploadUrl: ENV.api.apiFileUpload,
  isUploading: false,
  ...
  didInsertElement: function() {
    ...
    Ember.$('section.errors').hide();

    Ember.$('#file-upload').on('change', function() {
      self.send('addFiles', this.files);
    });

    Ember.$('#image-upload-component').on('click', '#upload-trigger', function() {
      Ember.$('#file-upload').trigger('click');
    });

    Ember.$('.modal').on('click', function(){
      Ember.$('section.errors').hide();
    });
  },

  processRawFile: function(file) {
  ...
  processImage: function(file, minAspectRatio) {
    return new Ember.RSVP.Promise(function(resolve, reject) {
  ...
  actions: {
    ...
    uploadFiles: function() {
      ...
      if(attachments.length < 1) {
        ...
        Ember.$('section.errors').text('No file selected for upload.');
        return;
      }
